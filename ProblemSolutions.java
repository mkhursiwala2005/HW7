...
public class ProblemSolutions {
     ...
     *
     * Start with the completely unordered array
     * Divide the array into half into two arrays
     * Tell each array to merge sort itself (Recursive part of the algorithm)
     *
     * 1st case: if the array contains only one element, then we know it is already sorted so we just RETURN
     */
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right)
    {
        ...
        int i = left;
        int j = mid+1;
        int l = 0; // pointer for temporary array tempArr
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        ...
        if (mass >= 1 && mass <= 105) {
            if (asteroids.length >= 1 && asteroids.length <= 105) {
                for (int i = 0; i < asteroids.length; i++) {
                    if (asteroids[i] >= 1 && asteroids[i] <= 105) {
                        if (mass < asteroids[i]) {
                            destroyed = false;
                            break;
                        } else {
